...
interface AuthContextType {
  ...
  verifyOTP: (email: string, token: string, purpose?: 'signup' | 'login' | 'password_reset' | 'admin_login') => Promise<{ error: Error | null }>;
  resendOTP: (email: string, purpose?: 'signup' | 'login' | 'password_reset' | 'admin_login', userData?: any) => Promise<{ error: Error | null }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        const checkReferral = (retryCount = 0) => {
          getProfile(session.user.id).then((p) => {
            if (p) {
              ...
              const isUserAdmin = p.role === 'admin' || p.role === 'super_admin';
              if (isUserAdmin) {
  ...
  const signInWithEmail = async (email: string, password: string) => {
    try {
      ...
      const isUserAdmin = userProfile?.role === 'admin' || userProfile?.role === 'super_admin';
  ...
  const verifyOTP = async (email: string, token: string, purpose: 'signup' | 'login' | 'password_reset' | 'admin_login' = 'signup') => {
    try {
      ...
      if (data?.redirect_url) {
        // For admin OTP login, we get a magic link back
  ...
  const resendOTP = async (email: string, purpose: 'signup' | 'login' | 'password_reset' | 'admin_login' = 'signup', userData?: any) => {
    try {
