...
interface RequestBody {
  email: string;
  purpose: 'signup' | 'login' | 'password_reset' | 'admin_login';
  userData?: {
    fullName: string;
    phone: string;
    country: string;
...
serve(async (req) => {
  ...
  try {
    ...
    // --- SECURITY: Role Check for Admin Login ---
    if (purpose === 'admin_login') {
      ...
      if (profileError || !profile || !['admin', 'super_admin'].includes(profile.role)) {
        return new Response(
          JSON.stringify({ error: 'Unauthorized: Admin role required for this login method' }),
    ...
    console.log(`[send-otp] Generated OTP for ${email}: ${otpCode}`);
