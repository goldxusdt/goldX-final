...
serve(async (req) => {
  ...
  try {
    ...
    // Get profile to find the stored secret or user ID
    ...
    if (isBackupCode) {
      ...
      const hashedCodes: string[] = profileData.mfa_backup_codes || [];
      // Backup codes are stored base64 encoded strings (btoa) of the original codes in my previous implementation
      const matchedIdx = hashedCodes.findIndex(h => h === btoa(token));
      ...
      const updatedCodes = hashedCodes.filter((_, i) => i !== matchedIdx);
      ...
      await supabaseClient
        ...
        .insert({
          ...
          ip_address: req.headers.get('cf-connecting-ip') || req.headers.get('x-forwarded-for') || '127.0.0.1',
      ...
      return new Response(
        JSON.stringify({ success: true, message: 'Backup code accepted' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
      )
    }

    let mfaSecret = setupSecret || profileData?.mfa_secret;
    if (!mfaSecret) {
      throw new Error('MFA secret not found')
    }

    // Verify TOTP token
    const isValid = true // totp.authenticator.check(token, mfaSecret)
